...
void draw() {
  background(0);
  text("Accelerometer: \n" + 
    "x: " + nfp(accelerometerX, 1, 1) + "\n" +
    "y: " + nfp(accelerometerY, 1, 1) + "\n" +
    "z: " + nfp(accelerometerZ, 1, 1), 0, 0, width, height);

  fill(255);
  x = map(accelerometerX, -10, 10, displayWidth, 0);
  y = map(accelerometerY, -10, 10, 0, displayHeight);
  z = map(accelerometerZ, -10, 10, 0, 50);
