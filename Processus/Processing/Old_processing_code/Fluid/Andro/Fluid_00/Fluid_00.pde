...
void draw() {
  //////////////////////////////////
  float newX = constrain(map(accelerometerX, -10, 10, width, 0), 0, width);
  float newY = constrain(map(accelerometerY, -10, 10, 0, height), 0, height);
  //////////////////////////////////

  int mousex = int(newX) / PIXELS;
  int mousey = int(newY) / PIXELS;
  ...
  for (int i=0; i<N; i++) {
    ...
    particle.z *= 0.99;
    particle.z = max(particle.z, pow((abs(velx) + abs(vely)), 0.75)*RADIUS/4);
